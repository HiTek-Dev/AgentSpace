...
must_haves:
  truths:
    ...
    - "InlineDiff component renders red/green diffs when provided old/new text strings, and is wired into MessageBubble via heuristic detection of diff-like tool output"
    - "Long diffs are auto-collapsed with a summary showing line count; user can press Enter to expand"
...
<task type="auto">
  ...
  <action>
    **Create InlineDiff.tsx** -- Red/green line diff display with collapsible behavior:
    ...
    Props: `{ oldText: string, newText: string, fileName: string, isFocused?: boolean }`
    ...
    - **Internal expand/collapse state:** Use `useState(false)` for `expanded`. Auto-collapsed threshold: diffs with >20 changed lines start collapsed (`expanded` defaults to `false`); diffs with <=20 lines start expanded (`expanded` defaults to `true`). Compute initial state in a `useMemo` based on changed line count.
    - **Toggle mechanism:** Use `useInput` with `isActive={isFocused ?? false}`. When the user presses **Enter** key, toggle `expanded` state (`setExpanded(prev => !prev)`). The parent component (MessageBubble or ConversationScroll) passes `isFocused` to indicate this diff is the active/selected element. For the MVP, the most recent diff in the conversation is auto-focused (`isFocused={true}`), allowing the user to press Enter to expand it. Older diffs are not focused and show in their default state.
    - **Collapsed view** (when `!expanded`):
      - Render: `[pencil icon] fileName (N lines changed) [press Enter to expand]` in blue with dimmed count and dimmed hint
      - Single line, compact
    - **Expanded view** (when `expanded`):
      - Render file header: `[pencil icon] fileName` in blue bold, with dimmed `[Enter to collapse]` hint
      - For each change:
    ...
    **Update MessageBubble.tsx** -- Optional diff enhancement:
    ...
    - Pass `isFocused={isLastToolCall}` to InlineDiff where `isLastToolCall` is true only for the most recent tool call message in the conversation. This enables the Enter-to-expand toggle for the latest diff. Older diffs render in their default collapsed/expanded state without keyboard interaction.
  ...
  <done>
    InlineDiff component renders red/green line changes with auto-collapse for large diffs and Enter-key toggle to expand/collapse. TodoPanel displays with tree-style layout and Unicode status icons matching Claude Code aesthetic. Diff rendering is wired into MessageBubble for tool call outputs that contain diff-like content (heuristic detection). The most recent diff is focusable for keyboard-driven expand/collapse.
  </done>
</task>
...
- File diffs render with red/green coloring when diff-like tool output is detected via heuristic
- Large diffs auto-collapse with summary count; Enter key toggles expand/collapse on the focused diff
