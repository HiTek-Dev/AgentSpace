...
must_haves:
  truths:
    ...
    - "CLI displays bash commands, tool calls, and reasoning inline with distinct styling per message type (covers CLI-02; types defined in Phase 3, populated in Phase 6)"
  ...
  artifacts:
    - path: "packages/cli/src/lib/gateway-client.ts"
      provides: "ChatMessage discriminated union with text, tool_call, bash_command, reasoning types"
      exports: ["ChatMessage", "TextMessage", "ToolCallMessage", "BashCommandMessage", "ReasoningMessage"]
    - path: "packages/cli/src/components/MessageBubble.tsx"
      provides: "Single message display with type-based styling (text, tool_call, bash_command, reasoning)"
...
3. **Extend ChatMessage type in `packages/cli/src/lib/gateway-client.ts`** (modifying Plan 03-01 artifact):
   - Change `ChatMessage` from a flat interface to a discriminated union on a `type` field:
     ```ts
     type ChatMessageBase = { id: string; timestamp: string };
     type TextMessage = ChatMessageBase & { type: "text"; role: "user" | "assistant" | "system"; content: string };
     type ToolCallMessage = ChatMessageBase & { type: "tool_call"; toolName: string; input: string; output?: string; status: "pending" | "complete" | "error" };
     type BashCommandMessage = ChatMessageBase & { type: "bash_command"; command: string; output?: string; exitCode?: number };
     type ReasoningMessage = ChatMessageBase & { type: "reasoning"; content: string };
     export type ChatMessage = TextMessage | ToolCallMessage | BashCommandMessage | ReasoningMessage;
     ```
   - Update all existing `ChatMessage` creation sites (in useChat, useSlashCommands) to include `type: "text"` so existing functionality is unchanged.
   - **Note:** Phase 6 (tool use integration) will populate `tool_call` and `bash_command` messages. Phase 3 only creates `text` messages. The type structure is defined now for forward compatibility.
   - Also add `packages/cli/src/lib/gateway-client.ts` to this plan's `files_modified` frontmatter.

4. Create `packages/cli/src/components/MessageBubble.tsx`:
   - Import React, `Box`, `Text` from `ink`
   - Import `ChatMessage` from `../lib/gateway-client.js`
   - Import `MarkdownRenderer` from `./MarkdownRenderer.js`
   - Props: `{ message: ChatMessage }`
   - Render based on `message.type` discriminant:
     - `text` with role `user`: `<Box><Text bold color="cyan">{"❯ "}</Text><Text>{message.content}</Text></Box>` — user messages displayed as plain text with a prompt indicator
     - `text` with role `assistant`: `<Box flexDirection="column"><Text bold color="magenta">{"◆ Assistant"}</Text><MarkdownRenderer content={message.content} /></Box>` — full markdown rendering for completed assistant messages
     - `text` with role `system`: `<Box><Text color="yellow" dimColor>{"⚠ "}{message.content}</Text></Box>` — dimmed yellow for system/error messages
     - `tool_call`: `<Box flexDirection="column"><Text bold color="blue">{"🔧 Tool: "}{message.toolName}</Text><Text dimColor>{message.input}</Text>{message.output && <Text>{message.output}</Text>}</Box>` — styled tool call display (will be unpopulated until Phase 6)
     - `bash_command`: `<Box flexDirection="column"><Text bold color="green">{"$ "}{message.command}</Text>{message.output && <Text dimColor>{message.output}</Text>}</Box>` — terminal-style bash display (will be unpopulated until Phase 6)
     - `reasoning`: `<Box><Text dimColor italic>{"💭 "}{message.content}</Text></Box>` — dimmed italic for reasoning traces (will be unpopulated until Phase 6)
