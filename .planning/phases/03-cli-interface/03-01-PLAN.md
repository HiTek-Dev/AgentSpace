...
must_haves:
  truths:
    ...
    - "User sees connection status update in real-time as the WebSocket connects and disconnects from the gateway"
    - "Messages stream and update continuously during assistant responses, with usage totals tracked across the session"
