...
<task type="auto">
  ...
  <action>
    1. In `packages/gateway/src/ws/handlers.ts`, add a module-level migration flag and call:
       ...
       Also in `handleChatSend()`, extract the agentId from the loaded agent config (e.g., `state.agentConfig?.id ?? "default"`) and pass it to `assembleContext()`:
       ```typescript
       const agentId = state.agentConfig?.id ?? "default";
