...
3. **Update gateway-client.ts** at `packages/cli/src/lib/gateway-client.ts`:
  ...
  <verify>
    NOTE: `gateway-client.ts` imports `ClientMessage` from `@tek/gateway`. The CLI tsc will only pass after Task 2 adds `agentId` to `protocol.ts`. Run gateway build first:
    Run `cd /Users/hitekmedia/Documents/GitHub/tek && npx tsc -p packages/gateway/tsconfig.json --noEmit` — must pass (requires Task 2 protocol.ts change).
...
2. **Update handlers.ts** at `packages/gateway/src/ws/handlers.ts`:
   ...
   - Also check if the tool registry needs to be rebuilt when agentId changes. Currently the tool registry is cached on `connState.tools`. If the agentId from the message differs from the agentId used to build the cached tools, the tools should be rebuilt. Add logic:
     ...
     ```
     Add `lastAgentId?: string` to the `ConnectionState` type in `packages/gateway/src/ws/connection.ts`.
