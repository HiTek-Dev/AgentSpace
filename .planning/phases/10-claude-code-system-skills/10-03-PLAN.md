...
user_setup:
  ...
  - service: stability-ai
    why: "Image generation via Stability AI (SDXL) as alternative to OpenAI"
    env_vars:
      - name: STABILITY_API_KEY
        source: "Sign up at platform.stability.ai -> Account -> API Keys"
    dashboard_config:
      - task: "Create Stability AI account and generate API key"
        location: "https://platform.stability.ai/account/keys"
...
must_haves:
  truths:
    ...
    - "Agent can generate images from text prompts via OpenAI or Stability AI"
  ...
  artifacts:
    ...
    - path: "packages/gateway/src/skills/image-gen.ts"
      provides: "Image generation AI SDK tools using OpenAI gpt-image-1.5 and Stability AI SDXL"
      exports: ["createImageGenTool", "createStabilityImageGenTool"]
    ...
    - path: "packages/gateway/src/agent/tool-registry.ts"
      ...
      contains: "createWebSearchTool|createImageGenTool|createStabilityImageGenTool"
  key_links:
...
Also export `createStabilityImageGenTool(apiKey: string)` returning an AI SDK tool:
  - `description`: "Generate an image from a text description using Stability AI."
  - `inputSchema`: `z.object({ prompt: z.string().describe("Detailed description of the image to generate"), negativePrompt: z.string().optional().describe("What to exclude from the image"), aspectRatio: z.enum(["1:1", "16:9", "9:16", "4:3", "3:4"]).optional().default("1:1") })`
  - `execute`: async function that:
    1. Calls Stability AI REST API: `POST https://api.stability.ai/v2beta/stable-image/generate/sd3` with `multipart/form-data` body containing `prompt`, `negative_prompt` (if provided), `aspect_ratio`, `output_format: "png"`, `model: "sd3.5-large"`
    2. Set headers: `Authorization: Bearer ${apiKey}`, `Accept: "image/*"`
    3. Response is raw image bytes. Convert to base64 data URL: `data:image/png;base64,${base64}`
    4. Return `{ url: dataUrl }` (base64 data URL)
    5. On error: return `{ error: "Stability image generation failed: " + message }`
- No external SDK needed -- use raw fetch with FormData (keeps dependencies minimal)
  </action>
  <verify>
Run `npx tsc --noEmit` from packages/gateway. Verify both tool factories compile and return AI SDK tool objects.
  </verify>
  <done>Web search tool calls Tavily API and returns structured search results. Image generation tools: OpenAI gpt-image-1.5 returns image URL, Stability AI SDXL returns base64 data URL. All are AI SDK tool() definitions ready for registry.</done>
</task>
...
- Barrel export all: `createWebSearchTool`, `createImageGenTool`, `createStabilityImageGenTool`, `getPlaywrightMcpConfig`, `PLAYWRIGHT_SERVER_NAME`

Update `packages/gateway/src/agent/tool-registry.ts`:
- Import system skill factories from `../skills/index.js`
- Add `ToolRegistryOptions` fields: `tavilyApiKey?: string`, `openaiApiKey?: string`, `stabilityApiKey?: string`
- After existing tool registration (step 4 skill tools), add step 5:
  ...
  - If `stabilityApiKey` provided: create Stability image gen tool via `createStabilityImageGenTool(stabilityApiKey)`, add to tools map as `"stability_image_generate"`, apply approval wrapper (use "session" tier -- costs money)
...
Run `npx tsc --noEmit` from packages/gateway. Verify tool-registry.ts imports from skills and conditionally registers web_search and image_generate tools.
  ...
  <done>System skills (web search, OpenAI image gen, Stability AI image gen) are conditionally registered in the tool registry based on API key availability. Playwright browser automation works through existing MCP config with zero custom code. All system skills are available as agent tools.</done>
</task>
...
3. Tool registry conditionally includes `web_search`, `image_generate`, and `stability_image_generate` when API keys are configured
...
- OpenAI image generation tool accepts prompt and returns image URL from gpt-image-1.5
- Stability AI image generation tool accepts prompt and returns base64 data URL from SDXL
